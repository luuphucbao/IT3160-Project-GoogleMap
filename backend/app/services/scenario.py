...
class ScenarioService:
    ...
    def calculate_affected_edges(
        ...
        # 1. Lấy dữ liệu đồ thị từ RAM (Cực nhanh)
        nodes = pathfinding_service.nodes
        # Dùng original_weights để đảm bảo tính toán trên đồ thị gốc
        edges_dict = pathfinding_service.original_weights 
        ...
        for (u, v), _ in edges_dict.items():
            ...
            # --- TOÁN HỌC HÌNH CHIẾU (Giữ nguyên logic cũ) ---
            mid_x = (p1[0] + p2[0]) / 2
            mid_y = (p1[1] + p2[1]) / 2
    ...
    def add_scenario(self, scenario_data: Dict, affected_edges: List[Tuple[int, int]]):
        """Lưu kịch bản vào danh sách tạm"""
        new_scenario = {
            "id": self.counter_id,
            "active": True,
            **scenario_data,
            "affected_edges_list": affected_edges, # Lưu để sau này còn biết đường mà xóa
            "affected_edges": len(affected_edges)  # Số lượng cạnh (để hiển thị)
        }
