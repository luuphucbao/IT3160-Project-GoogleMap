...
from app.database import get_db_connection, create_session_table
...
class PathfindingService:
    ...
    def load_graph(self):
        ...
        with get_db_connection() as conn:
            ...
            # Load edges
            cursor.execute("SELECT node_from, node_to, weight FROM edges_session")
    ...
    def _reconstruct_path(self, came_from: Dict, current: int) -> Dict:
        ...
        # --- NEW: Calculate total cost from edge weights ---
        total_cost = 0
        for i in range(len(path) - 1):
            current_node_id = path[i]
            next_node_id = path[i+1]
            
            # Find the weight of the edge between current and next node
            edge_weight = next((weight for neighbor, weight in self.graph[current_node_id]['neighbors'] if neighbor == next_node_id), 0)
    ...
    def reload_graph(self):
        ...
        self.load_graph()
        end_time=time.time()
        print(f"Time reload graph: {end_time-start_time}")
...
def get_pathfinding_service() -> PathfindingService:
    ...
    if _pathfinding_service is None:
        # Khởi tạo bảng session TRƯỚC KHI tạo service để đảm bảo service load đúng dữ liệu
