...
class MapViewer(QGraphicsView):
    ...
    def mousePressEvent(self, event):
        ...
        if self._is_placing_traffic_light_icon:
            ...
            if cleared_point:
                self.clear_path()
                # Truyền point_type khi báo về MainWindow
                self.on_point_selected(cleared_type, -1, -1)
                event.accept()
                return
    ...
    def wheelEvent(self, event):
        ...
        if event.angleDelta().y() > 0:
            ...
            # Chỉ cho phép zoom-out nếu scale_factor > 0.25
            if self.scale_factor <= 0.01:
                return  # Không zoom nhỏ hơn 0.25x
            zoom_factor = zoom_out_factor

        # Nếu zoom-out, kiểm tra scale_factor sau khi nhân
        new_scale = self.scale_factor * zoom_factor
        if new_scale < 0.01:
            zoom_factor = 0.01 / self.scale_factor  # Đặt về đúng 0.25x
